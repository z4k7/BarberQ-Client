    ...
    if (!this.showOtpField) {
      ...
      this.http.post(`${this.backendURL}/user/register`, user).subscribe({
        next: () => {
          ...
          this.toastr.success('User details submitted successfully. Enter OTP.');
        },
        error: (err) => {
          const errorMessage = err.error ? err.error.message : 'Unknown error';
      ...
      this.http.post(`${this.backendURL}/user/verifyOtp`, { email: user.email, otp: enteredOTP }).subscribe({
        next: () => {
